...
async def override_heavydb_client():
    """
    ...
    try:
        ...
        with AppConfig.change_config_sources(new_source):
            with patch("heavyiq.langchain.heavydb.get_config", return_value=AppConfig().iq):  # type: ignore
                yield await HeavyDB.from_env_async()
...
async def override_heavydb_client_for_query_request(
...
async def override_heavydb_client_for_question_request(
...
async def override_valid_table_metadata_db_session(
