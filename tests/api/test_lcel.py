...
@override_config
def test_should_pass_lcel_nl_to_answer_endpoint(heavyiq_config, client, session_id):
    """
    ...
    response = client.post("/api/v1/lcel/nl-to-answer", json=payload)
    assert response.status_code == 200
    response_json = response.json()
    assert response_json["answer"]
    assert response_json["sql"]
    assert response_json["sql_result"]
    assert response_json["sql_complexity"] in [3, 4]


@override_config
def test_should_pass_lcel_sql_to_answer_endpoint(heavyiq_config, client, session_id):
    """
    Test LCEL answer endpoint.
    """
    ...
    response = client.post("/api/v1/lcel/sql-to-answer", json=payload)
    assert response.status_code == 200
    response_json = response.json()
    assert response_json["answer"]
    assert response_json["sql"]
