...
async def call_llm(request: CallLLMRequest) -> CallLLMResponse:
    ...
    response = await llm.apredict( prompt, stop=request.stop, guided_choice=request.guided_choice,  # Passed as part of kwargs callbacks=[NoopLangChainTracer(client=1)]
)
