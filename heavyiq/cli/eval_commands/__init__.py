...
async def generate_eval_dataset_csv(
    ctx: click.Context, questions_csv: str, batch_size: int, temperature: float, verbose: bool
) -> None:
    """
    A function that assists in generating an evaluation dataset CSV, which needs to be passed as input to the 'run_config_model_on_questions' command.
    """
    print(
        f'Running eval "generate_eval_dataset_csv" function with the following parameters,\ntemperature={temperature}\nbatch_size={batch_size}\ninput_csv={questions_csv}'
    )

    from heavyiq.langchain.utils import is_langsmith_active
    from heavyiq.lcel.chains.eval.eval_dataset_chain import chain

    logger = get_heavyiq_logger()

    if not await run_in_threadpool(os.path.exists, questions_csv):
        raise Exception(f"questions_csv does not exist: {questions_csv}")

    eval_id = uuid4().hex[:8]
    eval_str = f"dataset_{eval_id}"
    logger.info(f"Dataset ID: {eval_id}")
    config = get_config()

    rows_written: int = 0
    eval_dataset_csv: str = f"./eval/results/{eval_str}_results.csv"

    # csv must have columns: id (optional), db_id, tables, question, answer
    async with aiofiles.open(questions_csv, mode="r") as f, aiofiles.open(eval_dataset_csv, "a", newline="") as wf:
        csv_reader, writer = AsyncReader(f), AsyncWriter(wf, dialect="unix")
        header = await anext(csv_reader)
        has_id = "id" == header[0]  # type: ignore

        # write header
        header_data = ["db_name", "tables", "question", "sql", "sql_complexity", "error"]
        if has_id:
        ...
        while True:
            ...
            for row in rows:
                if has_id:
                    query_id, db_id, question = row
                else:
                    query_id = None
                    db_id, question = row
                batch_inputs.append({"db_id": db_id, "question": question, "query_id": query_id})
            if not batch_inputs:
                break
    ...
    print(f"Successfully written {rows_written} rows into {eval_dataset_csv}.")
