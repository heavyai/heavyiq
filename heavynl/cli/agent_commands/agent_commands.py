...
from heavynl.langchain.agents.convo_functions_agent import create_conversational_function_agent
...
def nconversational(
    ...
    sql_agent = create_conversational_function_agent(chat_llm=chat_llm, verbose=verbose)
    # start a loop that asks for input and then calls the agent, break the loop on EXIT
    is_first_message = True
    while True:
        question = input(HumanConvoMessage(is_first_message=is_first_message).colorize())
