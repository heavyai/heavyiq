...
def query(body: dict) -> dict:
    ...
    response = {"sql": res[chain.output_key], "sql_complexity": int(res[chain.output_complexity_key])}
    return response
...
def question(body: dict) -> dict:
    ...
    chain = NLtoAnswerChain.from_same_llm(llm=llm, database=db, verbose=True)
    chain_input = {chain.input_key: question, "tables": tables}
    res = log_chain_call(chain, chain_input, MODEL_NAME)
    return {
        "answer": res[chain.output_key],
        "sql": res[chain.output_sql_key],
        "sql_complexity": int(res[chain.output_sql_complexity_key]),
    }
