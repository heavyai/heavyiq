...
def print_gunicorn_args(settings: dict):
    ...
    filtered_args = {key: value.get() for key, value in settings.items() if key in important_keys}
...
def on_exit(server):
    ...
    shared_instance = SharedDictSingleton._instance
    if shared_instance and (shared_dict_manager := shared_instance._manager) and shared_dict_manager._state.value == 1:
